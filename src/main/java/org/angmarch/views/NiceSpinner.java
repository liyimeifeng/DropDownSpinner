...
public class NiceSpinner extends AppCompatTextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        int defaultPadding = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_spinner_r_t_b_padding,resources.getDimensionPixelSize(R.dimen.one_and_a_half_grid_unit));

        setGravity(Gravity.CENTER_VERTICAL | Gravity.START);
        int leftPadding = typedArray.getDimensionPixelSize(R.styleable.NiceSpinner_spinner_left_padding,resources.getDimensionPixelSize(R.dimen.three_grid_unit));
        setPadding(leftPadding, defaultPadding, defaultPadding,
                defaultPadding);
        setClickable(true);
        ...
        isShowHintText = typedArray.getBoolean(R.styleable.NiceSpinner_showHintText, false);
        if (isShowHintText) {
            selectedIndex = -1;
        }else{
            selectedIndex = 0;
        ...
        arrowDrawableTint = typedArray.getColor(R.styleable.NiceSpinner_arrowTint, getResources().getColor(R.color.black));
    ...
    private Drawable initArrowDrawable(int drawableTint) {
        ...
        //Android 为了优化系统性能，资源 Drawable 只有一份拷贝，修改了它，等于修改了所用引用它的地方，于是这里调用 mutate()，来打破这种共享状态
        Drawable drawable = ContextCompat.getDrawable(getContext(), arrowDrawableResId).mutate();
        if (drawable != null) {
...
//        selectedIndex = -1 ;
        ...
        Log.e(TAG, "setAdapterInternal: " + selectedIndex );
        if(selectedIndex>=0){
            setTextInternal(adapter.getItemInDataset(selectedIndex).toString());
        }
    }
